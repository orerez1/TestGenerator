...
def create_null_edge_case_test(
    function_in_name: str, param_name: str, test_number: str,
    return_type: str, class_name: str, java_function: str,
    sending_params: str, params: str, is_singleton: bool, param_type: str) -> str:
    """
    ...
    primitive_types = ["boolean", "char", "byte", "short", "int", "long", "float", "double"]
    ...
    lowered_param_name = param_name.lower() if len(param_name) <= 1 else param_name.lower()[0] + param_name.lower()[1:]

    # remove the test param from the test method parameters
    params = ";\n".join([param for param in params.split(";\n") if param == '' or param.split(" = ")[-2].split(" ")[-1] != lowered_param_name])
